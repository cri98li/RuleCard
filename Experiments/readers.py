...
def read_mushroom(encode=True):
    target_col = 'class'
    df = pd.read_csv("../datasets/CLF/mushrooms.csv")
    ...
    if encode:
        ...
        if len(col_to_one_hot) > 0:
            col_to_one_hot = list(col_to_one_hot)
            encoder = OneHotEncoder(handle_unknown='ignore')
            encoder.fit(df[(col_to_one_hot)].values)
            df[encoder.get_feature_names_out()] = encoder.transform(df[col_to_one_hot].values).todense()
            df = df.copy().drop(columns=col_to_one_hot)
...
def read_telco(encode=True):
    target_col = 'Churn'
    df = pd.read_csv("../datasets/CLF/telco.csv").drop(columns=['customerID'])
